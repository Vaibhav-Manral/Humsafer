...
import styles from "./AccuracyRawReport.module.css";
import { exportAccuracyRawReport } from "../../api/Reports";
...
interface AccuracyRawReportProps {
  onSubmit: (startDate: Date, endDate: Date) => void;
  reportType?: string;
}

const AccuracyRawReport: React.FC<AccuracyRawReportProps> = ({
  onSubmit,
  ...
  const onAccuracyRawReportRequest = async () => {
    ...
    if (startDate && endDate) {
      const response = await exportAccuracyRawReport(
        companyId,
      ...
      if (response instanceof HumsaferError) {
        ...
        toast.success("Accuracy raw report sent to registered email");
      }
    }
  };
      ...
      <Box className={styles.paymentInfo_container}>
        ...
        <Box className={styles.buttonContainer}>
          <Button
            variant="contained"
            color="primary"
            onClick={onAccuracyRawReportRequest}
            disabled={!startDate || !endDate}
            className={styles.submitButton}
          >
            <Loading
              text="Generate Accuracy Raw Report"
              isLoading={isLoading}
            />
          </Button>
        </Box>
...
export default AccuracyRawReport;